...
export async function GET(req: Request) {
  try {
    ...
    const baseInvoiceWhere: any = { deletedAt: null };
    const baseLeadWhere: any = { deletedAt: null };
    const baseWipWhere: any = { deletedAt: null };
    const baseFinalCheckWhere: any = { deletedAt: null };
    ...
    const [totalInvoices, activeInvoices, deliveriesThisWeek, overdueInvoices] = await Promise.all([
      (prisma.invoice as any).count({ where: { ...baseInvoiceWhere, createdAt: { gte: startOfMonth, lte: endOfMonth } } }),
      (prisma.invoice as any).count({ where: { ...baseInvoiceWhere, status: "ACTIVE" } }),
    ...
    const [totalLeads, wipCounts, leadsByStatus] = await Promise.all([
      (prisma.lead as any).count({ where: { ...baseLeadWhere, createdAt: { gte: startOfMonth, lte: endOfMonth } } }),
    ...
    const [urgentDeliveries, recentInvoices, pendingFinalChecks, completedFinalChecksThisMonth] = await Promise.all([
      ...
      (prisma.finalCheck as any).count({ where: { ...baseFinalCheckWhere, isComplete: false } }),
